\# Claude Instructions — CMapr R package
\# Modern R Development Guide \## Core Principles
...
\## Modern Tidyverse Patterns \### Pipe Usage (`|>` not `%>%`) - \*\*Always use native pipe `|>` instead of magrittr `%>%`\*\* - R 4.3+ provides all needed features
...
| Simple combining \| `c()` \| `vec\_c()` \| Need type stability, consistent rules \|

| Custom classes \| S3 manually \| `new\_vctr()` \| Want data frame compatibility, subsetting \|

| Type conversion \| `as.\*()` \| `vec\_cast()` \| Need explicit, safe casting \|

| Finding common type \| Not available \| `vec\_ptype\_common()` \| Combining heterogeneous inputs \|

| Size operations \| `length()` \| `vec\_size()` \| Working with non-vector objects \|
...
\- \*\*Simple operations\*\* - `vec\_c(1, 2)` vs `c(1, 2)` for basic atomic vectors
...
\- \*\*Use `map() |> list\_rbind()`\*\* instead of superseded `map\_dfr()`
...
| \*\*Class definition\*\* \| Informal (convention) \| Formal (`new\_class()`) \| Need guaranteed structure \|
